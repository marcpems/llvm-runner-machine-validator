...
| 5 | Git for Windows installed and first on PATH | Wrong git.exe / missing Git for Windows broke checkout/build tooling that shells out to `git`. |
| 6 | Bash available | Some LLVM release build/test steps shell out to `bash` (e.g. lit-driven test-suite scripts and helper wrappers). If `bash.exe` isn't resolvable, those steps fail with `'bash' is not recognized` partway through a multi-hour build/test run. Bash normally ships with Git for Windows at `C:\Program Files\Git\bin\bash.exe`. |
| 7 | 7-Zip (`7z.exe`) installed | The final release-packaging step failed with `'7z' is not recognized as an internal or external command` **after** a full multi-hour build+test run had already completed. |
| 8 | 7-Zip directory present on machine-level PATH | `7z.exe` existed but wasn't reachable, same failure as above. **Known quirk:** a machine PATH change is not picked up by an already-running process (including an already-running runner) until it is restarted - the script calls this out explicitly. |
| 9 | GitHub Actions Runner process running | If the listener isn't running, the machine can't pick up any dispatched job at all. |
| 10 | ASan known-failing-test exclusion overlay (git hook) installed | 5 specific ASan interceptor tests are known-failing in this environment (not real code regressions). A machine-local, never-committed `post-checkout` git hook appends them to `LIT_FILTER_OUT` in `build_llvm_release.bat` after every checkout, since that script hard-overwrites the variable with a literal string. This is a transient, per-machine overlay - it never touches the repo/branch. **Intel (x64) only** - auto-passes (skipped) on ARM64, since these ASan tests are not built/run there and the overlay is unnecessary. |
...
* **Detect-only by default.** Nothing changes on the machine unless you pass
  `-ApplyFixes`.
* **Architecture-aware.** Checks #2 and #10 exist solely to prevent ASan interceptor
