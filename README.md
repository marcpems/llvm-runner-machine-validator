...
| 17 | WiX Toolset v3 (`candle.exe`/`light.exe`) installed and on PATH | CPack's WIX generator (used to build the `.msi` installer) shells out directly to WiX v3's `candle.exe`/`light.exe`. Missing = `CMake Error ... Could not find the WiX candle executable` / `CPack Error: Fatal WiX Generator Error`, failing at the very last packaging step **after** the entire multi-hour build has already completed. WiX v4+ does not provide these executables (replaced by `wix build`), so a newer WiX install does not satisfy this. Not published on winget - auto-fix installs via Chocolatey (`choco install wixtoolset`, see checks #1/#2) and adds its `bin` directory to PATH if needed. |
| 18 | GitHub Actions Runner process running | If the listener isn't running, the machine can't pick up any dispatched job at all. |
| 19 | Windows power plan set to High performance | The default "Balanced" (or "Power saver") plan throttles CPU clock speed and parks cores to save power, which can noticeably slow down a multi-hour LLVM build/test run and add run-to-run timing variance. Auto-fix activates the built-in "High performance" scheme directly by its well-known GUID (works even when the plan is hidden from the Power Options UI, without needing to unhide/duplicate it first). |
| 20 | ASan known-failing-test exclusion overlay (git hook) installed | 5 specific ASan interceptor tests are known-failing in this environment (not real code regressions). A machine-local, never-committed `post-checkout` git hook appends them to `LIT_FILTER_OUT` in `build_llvm_release.bat` after every checkout, since that script hard-overwrites the variable with a literal string. This is a transient, per-machine overlay - it never touches the repo/branch. **Intel (x64) only** - auto-passes (skipped) on ARM64, since these ASan tests are not built/run there and the overlay is unnecessary. |
...
* **Detect-only by default.** Nothing changes on the machine unless you pass
  `-ApplyFixes`.
* **Architecture-aware.** Checks #4 and #20 exist solely to prevent ASan interceptor
